...
class LavalinkNodeManager {
    constructor(client) {
        ...
        this.voiceStatusSupportLogged = false;
    ...
    // ========== CORRECTED: Voice Channel STATUS (not name) ==========

    async setVoiceStatusValue(voiceChannel, statusText) {
        if (!voiceChannel) return false;

        if (typeof voiceChannel.setVoiceChannelStatus === 'function') {
            await voiceChannel.setVoiceChannelStatus(statusText);
            return true;
        }

        if (!this.voiceStatusSupportLogged) {
            console.log(
                `${colors.cyan}[ VOICE ]${colors.reset} ${colors.yellow}discord.js does not expose setVoiceChannelStatus() here, using REST fallback instead.${colors.reset}`
            );
            this.voiceStatusSupportLogged = true;
        }

        if (!this.client?.rest?.put) {
            throw new Error('Discord REST client is unavailable for voice status updates');
        }

        await this.client.rest.put(`/channels/${voiceChannel.id}/voice-status`, {
    ...
    /**
     * Update voice channel STATUS (what appears when clicking the voice channel)
     * @param {string} guildId - Guild ID
     * @param {string} songTitle - Current song title
     * @param {string} artist - Artist name
     */
    async updateVoiceChannelStatus(guildId, songTitle, artist = '') {
        try {
            ...
            console.log(`${colors.cyan}[ VOICE ]${colors.reset} ${colors.green}Updated voice channel status to: ${statusText}${colors.reset}`);
        } catch (error) {
            // Ignore permission errors
            if (error.code !== 50001) {
                console.error(`${colors.cyan}[ VOICE ]${colors.reset} ${colors.red}Failed to update voice channel status: ${error.message}${colors.reset}`);
            }
        }
    }
    
    /**
     * Reset voice channel STATUS (clear the "Listening to..." text)
     * @param {string} guildId - Guild ID
     */
    async resetVoiceChannelStatus(guildId, voiceChannelId = null) {
        try {
            const channelId = voiceChannelId || this.riffy?.players?.get(guildId)?.voiceChannel;
            if (!channelId) return;
            
            const voiceChannel = this.client.channels.cache.get(channelId);
            if (!voiceChannel) return;
            
            // Clear the voice channel status (set to null)
            await this.setVoiceStatusValue(voiceChannel, null);
            
            console.log(`${colors.cyan}[ VOICE ]${colors.reset} ${colors.yellow}Cleared voice channel status${colors.reset}`);
        } catch (error) {
            if (error.code !== 50001) {
                console.error(`${colors.cyan}[ VOICE ]${colors.reset} ${colors.red}Failed to clear voice channel status: ${error.message}${colors.reset}`);
            }
        }
    }
}
