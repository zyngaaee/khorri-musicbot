...
class StatusManager {
    ...
    async setDefaultStatus() {
        ...
        const update = async () => {
            ...
            const totalUsers = this.client.guilds.cache.reduce((acc, guild) => acc + (guild.memberCount || 0), 0);
            ...
            try {
                ...
                // Silently ignore errors during presence updates to avoid console noise
            }
        };
        
        await update();
        this.currentInterval = setInterval(update, 15000);
    }
